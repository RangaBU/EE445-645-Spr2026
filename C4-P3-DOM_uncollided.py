...
def compute_first_collision_source(
    ...
    Q1  (slide 18) -- from the direct solar beam
        The solar beam is one collimated direction, so no quadrature sum:

            Q1[k+0.5, Omega_{ij}] = (1/pi) * Gamma(Omega_solar -> Omega_{ij})
                                            * I0_dir_centre[k+0.5]

        Evaluated for ALL output directions Omega_{ij}  (i=1..N).

    Q2  (slide 19) -- from upward ground-reflected photons
        Sum over UPWARD source directions n=N/2+1..N  (mu_n > 0):

            Q2[k+0.5, Omega_{ij}] = (1/pi)
                * sum_{n=N/2+1}^{N} sum_{m=1}^{M}
                    w_n * w_phi * Gamma(Omega_{nm} -> Omega_{ij})
                                * I0_dif_centre[n,m,k+0.5]

        Evaluated only for UPWARD output directions  (i=N/2+1..N).
        (Slide 19: "This is done only for upward directions.")

    Q3  (slide 20) -- from all uncollided diffuse radiation
        Sum over ALL N quadrature directions  (n=1..N):

            Q3[k+0.5, Omega_{ij}] = (1/pi)
                * sum_{n=1}^{N} sum_{m=1}^{M}
                    w_n * w_phi * Gamma(Omega_{nm} -> Omega_{ij})
                                * I0_dif_centre[n,m,k+0.5]

        Evaluated for ALL output directions  (i=1..N).
        I0_dif contains both downward (sky) and upward (ground-reflected)
        uncollided photons, so this sums over both hemispheres.
    ...
    first_collision_source_Q1    : ndarray (N+1, M+1, K+1)  from direct sun
    first_collision_source_Q2    : ndarray (N+1, M+1, K+1)  from upward field
    first_collision_source_Q3    : ndarray (N+1, M+1, K+1)  from diffuse sky
    """
    K = num_layers_K
    ...
    for k in range(1, K + 1):        # loop over K cell centres
        ...
        # ── Q2: upward ground-reflected -> upward output only  (slide 19) ────
        # Source directions: n = N/2+1..N  (upward, mu_n > 0)
        # Output directions: i = N/2+1..N  (upward only, per slide 19)
        for i in range(N // 2 + 1, N + 1):
            for j in range(1, M + 1):
                weighted_sum = 0.0
                for n in range(N // 2 + 1, N + 1):    # upward source directions
                    for m in range(1, M + 1):
                        weighted_sum += (
                            polar_weights[n]
                            * azimuthal_weight
        ...
        # ── Q3: all diffuse radiation -> all output directions  (slide 20) ────
        # Source directions: n = 1..N  (ALL directions, both hemispheres)
        # Output directions: i = 1..N  (ALL directions)
        for i in range(1, N + 1):
            for j in range(1, M + 1):
                weighted_sum = 0.0
                for n in range(1, N + 1):              # all source directions
                    for m in range(1, M + 1):
                        weighted_sum += (
                            polar_weights[n]
                            * azimuthal_weight
...
if __name__ == "__main__":
    ...
    for band_label, band_data in spectral_bands.items():
        ...
        # Save the figure in the same folder as this script file.
        # os.path.dirname(__file__) gives the folder containing dom_uncollided.py,
        # so the output goes wherever the student keeps their code.
